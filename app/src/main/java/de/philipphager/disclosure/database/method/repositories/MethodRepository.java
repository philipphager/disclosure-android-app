...
public class MethodRepository {
    ...
    public Observable<List<Method>> byPermission(BriteDatabase db, String permissionId) {
      SqlDelightStatement selectByPermission = ProtectedMethod.FACTORY.selectByPermission(permissionId);
      CursorToListMapper<Method> cursorToList = new CursorToListMapper<Method>(MAPPER);

      return db.createQuery(selectByPermission.tables, selectByPermission.statement, selectByPermission.args)
          .map(SqlBrite.Query::run)
          .map(cursorToList);
    }
  }
